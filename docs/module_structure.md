...
| `sources.py` | 613 | All external I/O + local cache | `load_universe`, `load_aliases`, `load_sec_ticker_map`, `fetch_news`, `fetch_price_stats`, `fetch_stooq_price_stats`, `fetch_recent_sec_filings`, `fetch_sec_company_facts`, `EVENT_KEYWORDS` |
...
| `portfolio.py` | 507 | Paper validation ledger (SQLite) | `apply_paper_buy`, `update_portfolio_performance`, `archive_report`, `append_*_to_outputs` |
